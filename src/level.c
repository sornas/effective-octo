...
static f32 distance_func(f32 t, f32 a) {
    return sin(a * t) + sin(a * 0.5 * t) + 3.0;
}

static f32 angle_func(f32 t) {
    return t + t * sin(t) * 0.2;
}

static Vec2 calculate_point(f32 t, f32 a) {
    f32 angle = angle_func(t);
    f32 distance = distance_func(t, a);
...
LevelPointList generate_level_point_list() {
    static f32 a = 6.0;
    fog_util_tweak_f32("Noise", &a, 0.1);
    ...
    for (u32 i = 0; i < num_samples; i++) {
        f32 t = step * i + offset;
        Vec2 p = calculate_point(t, a);
        Vec2 start = calculate_point(t - step * 0.1, a);
        Vec2 end = calculate_point(t + step * 0.1, a);
...
void draw_level_point_list(LevelPointList *list) {
    static f32 strength = 4;
    fog_util_tweak_f32("Smoothness", &strength, 0.4);
    for (u32 i = 0; i < list->num_points; i++) {
        ...
        for (f32 t = 0; t < 1.0; t += 0.01) {
            f32 x = fog_std_progress_func_f32(p1.x, d1.x * strength, p2.x, d2.x * strength, t);
            f32 y = fog_std_progress_func_f32(p1.y, d1.y * strength, p2.y, d2.y * strength, t);
